...
impl fmt::Display for FunctionCallError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FunctionCallError::Utf8Error(e) => write!(f, "UTF-8 error: {e}"),
            FunctionCallError::UnexpectedEof => write!(f, "unexpected EOF"),
            FunctionCallError::InvalidLength => write!(f, "invalid length or format code"),
            FunctionCallError::InvalidFormatCode(c) => write!(f, "invalid format code: {c}"),
            FunctionCallError::UnexpectedTag(t) => write!(f, "unexpected tag: {t:#X}"),
        }
    }
}

impl StdError for FunctionCallError {
    fn source(&self) -> Option<&(dyn StdError + 'static)> {
        match self {
            FunctionCallError::Utf8Error(e) => Some(e),
            _ => None,
        }
    }
}
...
#[inline]
fn decode_format_code(code: i16) -> Result<bool, FunctionCallError> {
    match code {
        0 => Ok(false),
        1 => Ok(true),
        other => Err(FunctionCallError::InvalidFormatCode(other)),
    }
}
...
impl<'a> WireSerializable<'a> for FunctionCallFrame<'a> {
    ...
    fn from_bytes(mut bytes: &'a [u8]) -> Result<Self, Self::Error> {
        if bytes.remaining() < 5 {
            return Err(FunctionCallError::UnexpectedEof);
        }

        let tag = bytes.get_u8();
        if tag != b'F' {
            return Err(FunctionCallError::UnexpectedTag(tag));
        }

        let len = bytes.get_u32();
        if len < 4 || (len - 4) as usize != bytes.remaining() {
            return Err(FunctionCallError::InvalidLength);
        }
        ...
        for idx in 0..param_count {
            ...
            if is_binary {
                ...
                params.push(Parameter::Text(
                    str::from_utf8(slice).map_err(FunctionCallError::Utf8Error)?,
                ));
            }
        }
...
mod tests {
    ...
    fn invalid_tag() {
        ...
        matches!(err, FunctionCallError::UnexpectedTag(_));
    }
    ...
    fn invalid_format_code() {
        ...
        bytes[12] = 2; // 2
        let err = FunctionCallFrame::from_bytes(&bytes).unwrap_err();
        matches!(err, FunctionCallError::InvalidFormatCode(2));
    }
    ...
    fn invalid_result_format() {
        ...
        bytes[last] = 3;
        let err = FunctionCallFrame::from_bytes(&bytes).unwrap_err();
        matches!(err, FunctionCallError::InvalidFormatCode(3));
    }
    ...
    fn unexpected_eof() {
        ...
        let err = FunctionCallFrame::from_bytes(bytes).unwrap_err();
        matches!(err, FunctionCallError::UnexpectedEof);
    }
}
