...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ParseError::UnexpectedTag(t) => write!(f, "unexpected tag: {t:#X}"),
        }
    }
}
...
fn read_cstr<'a>(buf: &mut &'a [u8]) -> Result<&'a str, ParseError> {
    let nul = buf
        .iter()
        .position(|b| *b == 0)
    ...
    *buf = &rest[1..]; // skip NUL
    Ok(str::from_utf8(raw).map_err(ParseError::Utf8Error)?)
}
...
mod tests {
    ...
    fn invalid_tag() {
        ...
        matches!(err, ParseError::UnexpectedTag(_));
    }
    ...
    fn extra_data() {
        ...
        let err = ParseFrame::from_bytes(&bytes).unwrap_err();
        matches!(err, ParseError::InvalidLength);
    }
    ...
    fn invalid_utf8() {
        ...
        let err = ParseFrame::from_bytes(&bytes).unwrap_err();
        matches!(err, ParseError::Utf8Error(_));
    }
}
