...
impl<'a> CloseFrameObserver<'a> {
    ...
    pub fn peek(buf: &[u8]) -> Option<usize> {
        peek_tagged_frame(buf, b'C').map(|meta| meta.total_len)
    }
    ...
    pub fn new(frame: &'a [u8]) -> Result<Self, NewCloseObserverError> {
        let meta = match parse_tagged_frame(frame, b'C') {
            ...
            Err(TaggedFrameError::UnexpectedTag(tag)) => {
                return Err(NewCloseObserverError::UnexpectedTag(tag));
            }
        ...
        // name
        let rel =
            memchr(0, &frame[pos..meta.total_len]).ok_or(NewCloseObserverError::UnexpectedEof)?;
        let name =
